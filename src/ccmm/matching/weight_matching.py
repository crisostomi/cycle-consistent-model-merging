...
def synchronized_weight_matching_V2(models, ps: PermutationSpec, method, symbols, combinations, max_iter=100):
    """
    Find a permutation to make the params in `models` match.
    ...
    for p_ix in tqdm(range(len(perm_names) - 1, -1, -1)):
        p = perm_names[p_ix]
        n = perm_sizes[p]

        # a, b in similarities[a][b] are such that the permutation maps B to A
        similarities = {
            symb: {other_symb: torch.zeros((n, n)) for other_symb in symbol_set.difference(symb)} for symb in symbol_set
        }
        ...
        uber_matrix = three_models_uber_matrix(similarities[a][b], similarities[a][c], similarities[b][c], perm_dim=n)
