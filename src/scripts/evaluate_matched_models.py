...
def run(cfg: DictConfig) -> str:
    ...
    updated_params = {
        symb: {other_symb: None for other_symb in symbols.difference(symb)}
        for symb in symbols
    }
    ...
    permutations = load_permutations(
        cfg.permutations_path / "permutations.json",
        factored=cfg.use_factored_permutations,
    )

    # combinations of the form (a, b), (a, c), (b, c), .. and not (b, a), (c, a) etc

    canonical_combinations = [
        (fixed, permutee) for (fixed, permutee) in all_combinations if fixed < permutee
    ]
    ...
    for fixed, permutee in canonical_combinations:
        ...
        models: Dict[str, LightningModule] = {
            map_model_seed_to_symbol(seed): load_model_from_artifact(
                logger.experiment, artifact_path(seed)
            )
            for seed in cfg.model_seeds
        }

        model_orig_weights = {
            symbol: copy.deepcopy(model.model.state_dict())
        ...
        updated_params[fixed][permutee] = apply_permutation_to_statedict(
            permutation_spec,
            permutations[fixed][permutee],
            models[permutee].model.state_dict(),
        )
...
def evaluate_interpolated_models(
    ...
    trainer = instantiate(
        cfg.train, enable_progress_bar=False, enable_model_summary=False
    )
...
@hydra.main(
    config_path=str(PROJECT_ROOT / "conf"), config_name="matching", version_base="1.1"
)
